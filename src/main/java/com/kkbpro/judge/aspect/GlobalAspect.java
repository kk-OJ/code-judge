package com.kkbpro.judge.aspect;
...
import com.kkbpro.judge.utils.ReUtil;
import com.kkbpro.judge.utils.StringUtil;
...
public class GlobalAspect {
     ...
     * 定义切入点
     */
    @Pointcut("@annotation(com.kkbpro.judge.annotation.GlobalInterceptor)")
    ...
    private void verifyAuth() {
        ...
        if(Authorization == null) throw new BusinessException(Result.error(401,"token为空！"));
        String[] keys = Authorization.split(" ");
        if(keys.length != 3 || !"Bearer".equals(keys[0])) throw new BusinessException(Result.error(401,"token格式错误！"));
        String checkInfo = AuthorizationUtil.checkKey(keys[1],keys[2]);
        if(!AuthorizationUtil.AccessKeyValidInfo.equals(checkInfo))
            throw new BusinessException(Result.error(403,checkInfo));
    }
}